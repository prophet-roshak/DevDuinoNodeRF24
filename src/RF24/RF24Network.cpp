...
uint8_t RF24Network::update(void) {
	...
	while (radio.isValid() && radio.available()) //&pipe_num) )
	{
		...
		RF24NetworkHeader& header =
				*reinterpret_cast<RF24NetworkHeader*>(frame_buffer);
...
/******************************************************************/
bool RF24Network::write(RF24NetworkHeader& header, const void* message,
		size_t len, uint16_t writeDirect) {
...
bool RF24Network::write(uint16_t to_node, uint8_t directTo) // Direct To: 0 = First Payload, standard routing, 1=routed payload, 2=directRoute to host, 3=directRoute to Route
	...
	if (directTo == TX_ROUTED && ok && conversion.send_node == to_node
			&& frame_buffer[6] > 64 && frame_buffer[6] < 192) {
	...
	if (ok && conversion.send_node != to_node
			&& (directTo == 0 || directTo == 3) && frame_buffer[6] > 64
			&& frame_buffer[6] < 192) {
...
bool RF24Network::logicalToPhysicalAddress(
...
const char* RF24NetworkHeader::toString(void) const {
	...
	sprintf_P(buffer, PSTR("id %04x from 0%o to 0%o type %c"), id, from_node,
			to_node, type);
	return buffer;
}
